  ...
  //Black Box Delete Component, currently adapted for changing video collection
    deleteVideo = index => {
      let newVideoCollection = [
        ...this.state.users.vidcollection.slice(0, index)
        ...this.state.users.vidcollection.slice(index + 1),
      ];
      this.setState({ users.vidcollection: newVideoCollection })
  
      Black Box Update Component, currently adapted for video collection
      updateVideo = (index, ride) => {
          let newVideoCollection = [
            ...this.state.users.vidcollection.slice(0, index), user,
            ...this.state.users.vidcollection.slice(index + 1)
          ];
          this.setState({
            users: newUsers,
  ...
  return (
    <div className="container">
      <header className="App-header">
        ...
        <div>
          <input value={this.state.user.username} onChange={e => { this.setState({ username: e.target.value }) }} />
          <input value={this.state.user.password} onChange={e => { this.setState({ password: e.target.value }) }} />
      ...
      <div className="banners">
        ...
        <div className="letterbox"><img src={"./components/MiddleBox.jpg"} />
        </div>
        <div className="letterbox"><img src={"./components/RightBox.jpg"} />
        </div>
        <div>
          <h3>Collect</h3>
        </div>
